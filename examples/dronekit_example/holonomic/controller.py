...
class Controller(object):
    ...
    def update(self):
        ...
        self.set_target(self.state_traj[0, self.index], self.state_traj[1, self.index], self.state_traj[2, self.index],
                        self.input_traj[0, self.index], self.input_traj[1, self.index], self.input_traj[2, self.index])
        # save
        self.simulation['time'] = np.c_[self.simulation['time'], self.time]
        self.simulation['position_ref'] = np.c_[self.simulation['position_ref'], self.state_traj[:, self.index]]
        self.simulation['velocity_ref'] = np.c_[self.simulation['velocity_ref'], self.input_traj[:, self.index]]
    ...
    def store_trajectories(self, input_traj, state_traj):
        self.input_traj_strg = np.c_[input_traj[0, :], -input_traj[1, :], -input_traj[2, :]].T
        self.state_traj_strg = np.c_[state_traj[0, :], -state_traj[1, :], -state_traj[2, :]].T
        self.new_trajectories = True

    def proceed(self):
        if self.simulation['position'].shape[1] == 0:
            return True
        print self.simulation['position'][:, -1]
    ...
    def init_plots(self):
        ...
        for k in range(3):
            self.axes['pose'].append(self.figs[-1].add_subplot(6, 1, k+4))
            self.axes['pose'][3+k].plot([], [], color=[17./255., 110./255., 138./255.])
        self.figs.append(plt.figure())
        self.axes['velocity'] = []
        for k in range(3):
    ...
    def update_plots(self):
        ...
        for k in range(3):
            self.axes['pose'][3+k].lines[0].set_data(self.simulation['time'], self.simulation['orientation'][k, :])
...
if __name__ == '__main__':
    ...
    controller.start(np.r_[10., 10., 15.])
    plt.show(block=True)
