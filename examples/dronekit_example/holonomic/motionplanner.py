...
class MotionPlanningThread(threading.Thread):
    ...
    def configure(self, sample_time, update_time):
        # create vehicle
        self.quad = omg.Holonomic3D(omg.Sphere(0.5))
        self.quad.set_options({'safety_distance': 0.1, 'safety_weight': 10, 'room_constraints': False})
        self.quad.set_initial_conditions([0, 0, 0])
        ...
        problem.set_options({'horizon_time': 10, 'solver_options': {'ipopt': {'ipopt.linear_solver': 'ma57'}}})
        problem.init()
