...
environment.add_obstacle(Obstacle({'position': [6,0]}, shape=rect))
# environment.add_obstacle(Obstacle({'position': [6,-2]}, shape=rect))
# environment.add_obstacle(Obstacle({'position': [6,2]}, shape=rect))

environment.add_obstacle(Obstacle({'position': [1, 0]}, shape=Rectangle(width=2, height=2), options={'bounce':False},
                                  simulation={'trajectories': {}}))

environment.add_danger_zone(DangerZone({'position': [0, -1]}, shape=Rectangle(width=4, height=4),
...
schedulerproblem.save_movie('scene', format='gif', name='passing_pos', number_of_frames=80, movie_time=4, axis=True)
vehicle.save_movie('input', format='gif', name='passing_vel', number_of_frames=80, movie_time=4, axis=True)