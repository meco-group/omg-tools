...
# create vehicle
vehicle = AGV(length=0.8, options={'plot_type': 'agv', 'stop_tol': 1e-2})
# vehicle.set_options({'safety_distance': 0.3})
vehicle.define_knots(knot_intervals=8)  # choose lower amount of knot intervals
...
problem0.set_options({'solver_options': {'ipopt': {'ipopt.linear_solver': 'ma57',
    'ipopt.hessian_approximation': 'limited-memory', 'ipopt.tol': 1e-3}}}) #for gridding comparison: ,'ipopt.tol': 1e-12
problem0.init()
...
# # for gridding comparison: 
# # vehicle.save_plot('state', name='state', knots=True)
# # vehicle.save_plot('input', name='input', knots=True)
# # problem.save_plot('scene', name='agv', knots=True)

# # problem.save_movie('scene', format='tikz', name='agv', number_of_frames=100, axis=False)
