...
if solver is 'ipopt':
	...
	worhp_options = {#'worhp.qp_ipLsMethod': 'MA57',  # todo: option not found?
					 'worhp.MaxIter': 200,
					 'worhp.TolOpti': 1e-6,
					 'worhp.InitialLMest': False,  # False = warm start
	...
	problem = Point2point(vehicle, environment, options, freeT=False)
	problem.set_options({'solver_options': {'snopt': {'snopt.Hessian': 'limited memory',
													  'snopt.start': 'Warm'}}})
else:
	print 'You selected solver: ', solver, ' but this solver is not supported. Choose between ipopt, worhp or snopt.'
problem.init()
