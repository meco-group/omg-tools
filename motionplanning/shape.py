...
class Circle(Shape2D):
    ...
    def prepareDraw(self):
        ...
        self.plt_co = np.vstack((self.radius*np.cos(s*2*np.pi), self.radius*np.sin(s*2*np.pi)))

    def getCheckPoints(self, position):
        return [position], self.radius

    def getCanvasLimits(self):
        return {'xlim': np.array([-self.radius, self.radius]), 'ylim': np.array([-self.radius, self.radius])}
...
class Polyhedron(Shape2D):
    ...
    def prepareDraw(self):
        self.plt_co = np.hstack((self.vertices, np.vstack(self.vertices[:,0])))

    def draw(self, orientation = 0.):
        return self.rotate(orientation, self.plt_co)

    def getCheckPoints(self, position):
        chck = [[position[0] + self.vertices[0,l], position[1] + self.vertices[1,l]] for l in range(self.n_faces)]
        return chck, 0.

    def getCanvasLimits(self):
        max_xy = np.amax(self.vertices, axis=1)
        min_xy = np.amin(self.vertices, axis=1)
        return {'xlim': np.array([min_xy[0], max_xy[0]]), 'ylim': np.array([min_xy[1], max_xy[1]])}
...
class Rectangle(Polyhedron):
    ...
    def getVertices(self):
        A       = np.zeros((4,2))
        B       = np.zeros((4,1))
        radius  = [0.5*self.height, 0.5*self.width, 0.5*self.height, 0.5*self.width]
        dth     = 0.5*np.pi
        for l in range(4):
            A[l,:] = np.array([np.sin(l*dth), np.cos(l*dth)])
            B[l]   = radius[l]
...
class Rocket(Rectangle):
    ...
    def prepareDraw(self):
        w       = self.width
        h       = self.height
        plt_x   = [-0.5*w, -0.25*w, 0.25*w, 0.5*w, 0.5*w, 0.25*w, -0.25*w, -0.5*w]
