...
class Vehicle(OptiChild, PlotLayer):
            ...
            if shape in hyperplanes:
                for k, hyperplane in enumerate(hyperplanes[shape]):
                    ...
                    for l, chck in enumerate(checkpoints):
                        con = 0
                        con += (a[0]*chck[0] + a[1]*chck[1])*(1.-tg_ha**2)*(1+tg_ha1**2)
                        con += (-a[0]*chck[1] + a[1]*chck[0])*(2*tg_ha)*(1+tg_ha1**2)
                        pos = [0, 0]  # next part gives an offset to input position e.g. for trailer position
                        pos[0] = position[0]*(1+tg_ha**2)*(1+tg_ha1**2) + offset*(1-tg_ha**2)*(1+tg_ha1**2)+offset1*(1-tg_ha1**2)*(1+tg_ha**2)  # real_pos*(1+tg_ha**2) nieuwe offset toegevoegd
                        ...
                        con += (-b+rad[l]+safety_distance-eps)*(1+tg_ha**2)*(1+tg_ha1**2)
    ...
    def predict(self, predict_time, sample_time):
        ...
        if self.options['ideal_prediction']:
            ...
            self.prediction['state'] = state[:, -1] #state geeft de states voor elk tijdstip (stap van sample_time) tot predict_time. De prediction van de state is dus de laatste kolom
