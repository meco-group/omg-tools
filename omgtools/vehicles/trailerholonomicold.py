...
class TrailerHolonomicOld(Vehicle):

    def __init__(self, lead_veh=None, shapes=Square(0.2), l_hitch=0.2, options=None, bounds=None):
        bounds = bounds or {}
        Vehicle.__init__(
...
#splines = [tg_ha_tr, x, y, tg_ha_veh]
    ...
    def define_trajectory_constraints(self, splines):
        ...
        # self.define_constraint(self.l_hitch*2*dtg_ha_tr*(1+tg_ha_veh**2)+(v_x*(2*tg_ha_veh*(1-tg_ha_tr**2)-(1-tg_ha_veh**2)*2*tg_ha_tr))-(v_y*((1-tg_ha_tr**2)*(1-tg_ha_veh**2)+4*tg_ha_tr*tg_ha_veh))-T*eps*(1+tg_ha_veh**2),-inf,0.)
        # self.define_constraint(-self.l_hitch*2*dtg_ha_tr*(1+tg_ha_veh**2)-(v_x*(2*tg_ha_veh*(1-tg_ha_tr**2)-(1-tg_ha_veh**2)*2*tg_ha_tr))+(v_y*((1-tg_ha_tr**2)*(1-tg_ha_veh**2)+4*tg_ha_tr*tg_ha_veh))-T*eps*(1+tg_ha_veh**2),-inf,0.)
        self.define_constraint(-self.l_hitch * 2 * dtg_ha_tr - v_x * 2 * tg_ha_tr + v_y * (1 - tg_ha_tr ** 2) - T * eps,
                               -inf, 0.)
        self.define_constraint(self.l_hitch * 2 * dtg_ha_tr + v_x * 2 * tg_ha_tr - v_y * (1 - tg_ha_tr ** 2) - T * eps,
                               -inf, 0.)
    ...
    def set_initial_conditions(self, theta, input=np.zeros(2)):
        # add complete state(6 elements) and input(3 elements)
        # state=[x_tr, y_tr, theta_tr, x_veh, y_veh, theta_veh]
        #input = [dx, dy, dtheta]
        state = np.zeros(6)
    ...
    def set_parameters(self, current_time):
        ...
        # \dot(\theta_1)) = self.prediction['input'][0]/self.l_hitch*(np.sin(self.prediction['state'][5]-self.prediction['state'][2]))+self.prediction['input'][0]/self.l_hitch*(np.cos(self.prediction['state'][5]-self.prediction['state'][2]))
        ##\dot(r_1)) = 0.5*\dot(\theta_1))*(1+parameters_tr['tg_ha_tr0']**2)
        ## state=[x_tr, y_tr, theta_tr, x_veh, y_veh, theta_veh] -> prediction is deze volgorde
         #input = [dx, dy, dtheta]
        parameters_tr['dtg_ha_tr0'] =0.5*(self.prediction['input'][0]/self.l_hitch*(np.sin(self.prediction['state'][5]-self.prediction['state'][2]))+self.prediction['input'][1]/self.l_hitch*(np.cos(self.prediction['state'][5]-self.prediction['state'][2])))*(1+parameters_tr['tg_ha_tr0']**2)
        if hasattr(self, 'theta_trT'):
    ...
    def ode(self, state, input):
        # state = [x_tr, y_tr, theta_tr, x_veh, y_veh, theta_veh]
        # input = [v_x,v_y, dtheta_veh]


        _, _, theta_tr, x_veh, y_veh, _ = state
        v_x, v_y, _ = input
