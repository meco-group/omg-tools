...
import numpy as np
 ...
 ##  1 assige trailer met knikpunt met niet-holomisch voertuig

class TrailerJoland(Vehicle):
#####
...
#####
        ...
        Vehicle.__init__(self, n_spl=1 + lead_veh.n_spl, degree=3, shapes=shapes,options=options)  # deze lijn na self.lead_veh (in tegenstelling tot trailer.py voor het geval er geen lead_veh gegeven zou zijn.
...
#####
    ...
    def define_trajectory_constraints(self, splines):
        ...
        dtg_ha_veh = tg_ha_veh.derivative()
        # change in orientation of the trailer is due to velocity of vehicle
        # relaxed this equality constraint with eps
        eps = 1e-3

        self.define_constraint(2*dtg_ha_tr*self.l_hitch1*((1+tg_ha_veh**2)**(2)) -
                               T*v_til_veh*(2*tg_ha_veh*(1-tg_ha_tr**2)-(1-tg_ha_veh**2)*2*tg_ha_tr)*((1+tg_ha_veh**2)**(2)) + (2*dtg_ha_veh*self.l_hitch)*((1-tg_ha_veh**2)*(1-tg_ha_tr**2)+2*tg_ha_veh*2*tg_ha_tr)- ((1+tg_ha_veh**2)**(2))*T*eps,
                               -inf, 0.)
        self.define_constraint(-2*dtg_ha_tr*self.l_hitch1*((1+tg_ha_veh**2)**(2)) +
                               T*v_til_veh*(2*tg_ha_veh*(1-tg_ha_tr**2)-(1-tg_ha_veh**2)*2*tg_ha_tr)*((1+tg_ha_veh**2)**(2)) - (2*dtg_ha_veh*self.l_hitch)*((1-tg_ha_veh**2)*(1-tg_ha_tr**2)+2*tg_ha_veh*2*tg_ha_tr)- ((1+tg_ha_veh**2)**(2))*T*eps,
                               -inf, 0.)
...
#####
#\dot(\theta_1)) = self.prediction['input'][0]/self.l_hitch1*(np.sin(self.prediction['state'][5]-self.prediction['state'][2]))-self.l_hitch/self.l_hitch1*(np.cos(self.prediction['state'][5]-self.prediction['state'][2]))*self.prediction['input'][1]
##\dot(r_1)) = 0.5*\dot(\theta_1))*(1+parameters_tr['tg_ha_tr0']**2) 


        parameters_tr['dtg_ha_tr0'] = 0.5*(self.prediction['input'][0]/self.l_hitch1*(np.sin(self.prediction['state'][5]-self.prediction['state'][2]))-self.l_hitch/self.l_hitch1*(np.cos(self.prediction['state'][5]-self.prediction['state'][2]))*self.prediction['input'][1])*(1+parameters_tr['tg_ha_tr0']**2) # dtg_ha
#####
        if hasattr(self, 'theta_trT'):
...
#####
        V_veh, dtheta_veh = input #dtheta_veh zal nodig zijn om volgende stappen te kunnen berekenen. 
        alpha = theta_veh-theta_tr
        dtheta_tr = V_veh/self.l_hitch1*np.sin(alpha) - self.l_hitch/self.l_hitch1 *np.cos(alpha)*dtheta_veh
