...
class TrailerJolandHolonomic(Vehicle):
    #####
    ...
    def define_trajectory_constraints(self, splines):
        ...
        #                         v_x *(1 + tg_ha_veh ** 2)* (2 * tg_ha_veh * (1 - tg_ha_tr ** 2) - (1 - tg_ha_veh ** 2) * 2 * tg_ha_tr)
        #                        -(v_y*(1+tg_ha_veh**2)-(2 * dtg_ha_veh * self.l_hitch)) * ((1 - tg_ha_veh ** 2) * (1 - tg_ha_tr ** 2) + 2 * tg_ha_veh * 2 * tg_ha_tr) - (
        #                        (1 + tg_ha_veh ** 2)) * T * eps,
        #                        -inf, 0.)
        # self.define_constraint(-2 * dtg_ha_tr * self.l_hitch1 * ((1 + tg_ha_veh ** 2) ** (2)) +
        #                         v_x * (1 + tg_ha_veh ** 2) * (
        #                        2 * tg_ha_veh * (1 - tg_ha_tr ** 2) - (1 - tg_ha_veh ** 2) * 2 * tg_ha_tr)
        #                        + (v_y * (1 + tg_ha_veh ** 2) - (2 * dtg_ha_veh * self.l_hitch)) * (
        #                        (1 - tg_ha_veh ** 2) * (1 - tg_ha_tr ** 2) + 2 * tg_ha_veh * 2 * tg_ha_tr) - (
        #                            (1 + tg_ha_veh ** 2)) * T * eps,
        #                        -inf, 0.)
    ...
    def check_terminal_conditions(self):
        ...
        if hasattr(self, 'theta_trT'):
            ...
            result = True
        result = (
        self.lead_veh.check_terminal_conditions() and result)  # arrived if lead_veh and trailer checks both True
        return result
